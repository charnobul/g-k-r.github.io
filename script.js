...
let farms = localStorage.getItem('farms') ? JSON.parse(localStorage.getItem('farms')) : [0, 0, 0, 0];
...
Array.from(buyButtons).forEach((buyButton, i) => {
    buyButton.addEventListener('click', function() {
        let costs = [1, 100, 10000, 500000];
        let incomes = [0.01, 1, 100, 50000];
        let cost = costs[i];
...
setInterval(function() {
    balance += 0.01 * farms[0] + 1 * farms[1] + 100 * farms[2] + 50000 * farms[3];
    updateBalance(balance);
}, 1000);
